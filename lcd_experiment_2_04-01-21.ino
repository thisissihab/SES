...
String inf = "";
unsigned long pv = 0;
bool comm_initiated = false, window_control_var = false;
...
void loop()
{
  ...
  if((millis() - pv > 1500) && comm_initiated){
    temp = "";
    comm_initiated = false;
  }

  if((millis() - manual_pv > 5000) && window_control_var){
  ...
  if(received_data){
      manual_pv = millis();
      mySerial.print("Access");
      window_control_var = true;
      received_data = false;
      if(!new_card){
      /*tft.fillScreen(GRAY);
